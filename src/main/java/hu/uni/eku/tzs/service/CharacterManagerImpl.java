...
public class CharacterManagerImpl implements CharacterManager {
    ...
    public Collection<Character> readAll() {
        ...
        return characterRepository.findAll().stream().map(CharacterManagerImpl::convertCharacterEntity2Model).collect(Collectors.toList());
    ...
    public Character modify(Character character) throws CharacterNotFoundException {
        ...
        {
            throw new CharacterNotFoundException(String.format("Can not found character with id %id", character.getId()));
        }
